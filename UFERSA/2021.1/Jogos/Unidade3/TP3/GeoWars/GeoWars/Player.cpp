...
{
	if ((lives != 0) && ((obj->Type() == ARROW) || (obj->Type() == BOOMERANG) || (obj->Type() == SHIP) || (obj->Type() == SHURIKEN) ||
	...
	if (!killed)
	{
		...
		if (gamepadOn)
		{
			...
			if (AxisTimed(AxisRX, AxisRY, 0.150f))
			{
				...
				GeoWars::audio->Play(SHOT);
				GeoWars::scene->Add(new Missile(ang), STATIC);
			}
		}
		...
		else
		{
			...
			{
				GeoWars::audio->Play(SHOT);
				GeoWars::scene->Add(new Missile(firingAngle), STATIC);
			}
		}
