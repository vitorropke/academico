...
void Camera::Update()
{
    ...
    {
        // mantém uma rotação constante no eixo x
        theta += 2.0f * float(frameTime);
    }
...
void Camera::Draw()
{
    ...
    // comando de desenho
    graphics->CommandList()->DrawIndexedInstanced(indexesNumber, 1, 0, 0, 0);
...
{
    // importa dados do arquivo .obj
    ifstream fin;

    fin.open("Resources/Objects/ball.obj");
    //fin.open("Resources/Objects/capsule.obj");
    //fin.open("Resources/Objects/cube.obj");
    //fin.open("Resources/Objects/house.obj");
    //fin.open("Resources/Objects/monkey.obj");
    //fin.open("Resources/Objects/pyramid.obj");
    //fin.open("Resources/Objects/thorus.obj");
    //fin.open("Resources/Objects/unknown.obj");
    //fin.open("Resources/Objects/vitor_name.obj");

    uint verticesNumber = 0;
    
    vector<vector<float>> dynamicVertices;
    vector<ushort> dynamicIndexes;

    // se a leitura do arquivo falhar, o programa fecha
    if (!fin.is_open())
    {
        exit(EXIT_FAILURE);
    }
    // se não falhar, continua com a leitura
    else
    {
        string line;
        bool hasNormal = false, hasTexture = false;
        ...
        while (getline(fin, line))
        {
            ...
            if ((line[0] == 'v') and (line[1] == ' '))
            {
                ...
                //     v         0              0              0
                iss >> c >> coordinateX >> coordinateY >> coordinateZ;

                dynamicVertices.push_back({ coordinateX, coordinateY, coordinateZ });
                verticesNumber++;
            }
            // flag do vértice de textura
            else if ((line[0] == 'v') and (line[1] == 't') and (line[2] == ' '))
            {
                hasTexture = true;
            }
            // flag do vértice normal
            ...
            {
                istringstream iss(line);
                ushort vertex, expendableNumber;
                char expendableChar;

                vector<ushort> currentIndexes;
                uint currentIndexesNumber = 0;

                iss >> expendableChar;
                while (!iss.eof())
                {
                    // dependendo da presença ou não da informação de textura e vetor normal, o formato da linha que define os índices, muda
                    // pode ter os seguintes formatos
                    // f 0/0/0
                    // f 0//0
                    // f 0/0/
                    // f 0 0 0
                    if (hasTexture and hasNormal)
                    {
                        //       0             /                  0                   /                 0
                        iss >> vertex >> expendableChar >> expendableNumber >> expendableChar >> expendableNumber;
                    }
                    else if (!hasTexture and hasNormal)
                    {
                        //       0             /                  /                  0
                        iss >> vertex >> expendableChar >> expendableChar >> expendableNumber;
                    }
                    else if (hasTexture and !hasNormal)
                    {
                        //       0             /                  0                  /
                        iss >> vertex >> expendableChar >> expendableNumber >> expendableChar;
                    }
                    else
                    {
                        //       0
                    ...
                    currentIndexes.push_back(vertex - 1);
                    currentIndexesNumber++;
                }

                // faz triangulação se a linha tiver 4 índices
                if (currentIndexesNumber == 4)
                {
                    dynamicIndexes.push_back(currentIndexes[0]);
                    dynamicIndexes.push_back(currentIndexes[1]);
                    dynamicIndexes.push_back(currentIndexes[3]);

                    dynamicIndexes.push_back(currentIndexes[3]);
                    dynamicIndexes.push_back(currentIndexes[1]);
                    dynamicIndexes.push_back(currentIndexes[2]);
    ...
    // se o fim do arquivo não foi alcançado, então alguma coisa não foi lida e o objeto não será feito. O programa fecha
    if (!fin.eof())
    {
        exit(EXIT_FAILURE);
    }
    ...
    {
        float x = dynamicVertices[i][0];
        float y = dynamicVertices[i][1];
        float z = dynamicVertices[i][2];
        ...
        vertices[i] = { XMFLOAT3(x, y, z), XMFLOAT4(randomColorR, randomColorG, randomColorB, 1.0f) };
    }

    // vetor para índices
    ushort* indices = new ushort[indexesNumber];
    for (uint i = 0; i < indexesNumber; i++)
    {
        indices[i] = dynamicIndexes[i];
    }
    ...
    const uint vbSize = verticesNumber * sizeof(Vertex);
    const uint ibSize = indexesNumber * sizeof(uint);
