...
LRESULT CALLBACK Input::InputProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_MOUSEMOVE:
		...
		// muda a cor do pixel por onde o mouse passa
		HDC hdc;
		hdc = GetDC(hWnd);

		SetPixel(
			hdc,				// contexto do dispositivo alvo
			mouseX,				// posição no eixo x
			mouseY,				// posição no eixo y
			RGB(255, 255, 255)	// cor do pixel
		);
