...
void Graphics::LogHardwareInfo()
{
	...
	*/
	...
	// ------------------------------------------
	// Resoluções suportadas pelo monitor
	// ------------------------------------------
	unsigned modesNumber = 0;
	output->GetDisplayModeList(DXGI_FORMAT_R8G8B8A8_UNORM, 0, &modesNumber, nullptr);

	DXGI_MODE_DESC* modesList = new DXGI_MODE_DESC[modesNumber];
	output->GetDisplayModeList(DXGI_FORMAT_R8G8B8A8_UNORM, 0, &modesNumber, modesList);

	wstringstream text;
	text << L"\n---> Resoluções suportadas:\n";
	OutputDebugStringW(text.str().c_str());

	for (unsigned i = 0; i < modesNumber; ++i) {
		unsigned screenWidth = modesList[i].Width;
		unsigned screenHeight = modesList[i].Height;
		unsigned numerator = modesList[i].RefreshRate.Numerator;
		unsigned denominator = modesList[i].RefreshRate.Denominator;

		wstringstream text;
		text << L"---> " << screenWidth << L"x" << screenHeight << L" " << numerator / denominator << " Hz\n";
		OutputDebugStringW(text.str().c_str());
	}
