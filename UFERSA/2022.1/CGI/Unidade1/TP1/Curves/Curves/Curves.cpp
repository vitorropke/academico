...
void Curves::Update()
{
    ...
    // cria vértices com o botão esquerdo do mouse
    if (input->KeyPress(VK_LBUTTON))
    {
        ...
        // depois que a função de Chaikin for executada, os próximos vértices serão vermelhos
        if (vertexDefaultColor)
            vertices[index] = { XMFLOAT3(x, y, 0.0f), XMFLOAT4(Colors::White) };
        else
            vertices[index] = { XMFLOAT3(x, y, 0.0f), XMFLOAT4(Colors::Red) };
    ...
    // realiza uma iteração do algoritmo de Chaikin com o pressionamento da tecla ENTER
    if (input->KeyPress(VK_RETURN))
    {
        // se a quantidade de novos vértices não ultrapassa o limite máximo a iteração é feita
        if ((2 * count - 2) < MaxVertex)
        {
            Vertex newVertices[MaxVertex]{};
            uint newIndex = 0;
            ...
            for (uint i = 0; i < count - 1; i++)
            {
                ...
                // converte a faixa -1.0 a 1.0 para as coordenadas da tela
                float coordenadaXTelaVerticeAtual = vertices[i].Pos.x * cx + cx;
                float coordenadaYTelaVerticeAtual = (vertices[i].Pos.y * cy - cy) * -1.0f;
                float coordenadaXTelaProximoVertice = vertices[i + 1].Pos.x * cx + cx;
                float coordenadaYTelaProximoVertice = (vertices[i + 1].Pos.y * cy - cy) * -1.0f;
                
                // calcula o ponto P(1/4)
                coordenadaX = (3.0f / 4.0f) * coordenadaXTelaVerticeAtual + (1.0f / 4.0f) * coordenadaXTelaProximoVertice;
                coordenadaY = (3.0f / 4.0f) * coordenadaYTelaVerticeAtual + (1.0f / 4.0f) * coordenadaYTelaProximoVertice;
                ...
                newVertices[newIndex] = { XMFLOAT3(x, y, 0.0f), XMFLOAT4(Colors::Red) };
                newIndex++;

                // calcula o ponto P(3/4)
                coordenadaX = (1.0f / 4.0f) * coordenadaXTelaVerticeAtual + (3.0f / 4.0f) * coordenadaXTelaProximoVertice;
                coordenadaY = (1.0f / 4.0f) * coordenadaYTelaVerticeAtual + (3.0f / 4.0f) * coordenadaYTelaProximoVertice;
            ...
            // atualiza o contador e o índice com o novo número de vértices
            index = count = 2 * count - 2;

            // coloca os novos vértices na estrutura principal 'vertices'
            for (uint i = 0; i <= index; i++)
                vertices[i] = newVertices[i];

            // define que os próximos vértices não terão a cor padrão
            vertexDefaultColor = false;
        }
        // se a quantidade de novos vértices ultrapassa o limite máximo a iteração não é feita e
        // a cor dos vértices muda para a cor padrão
        else
        {
            for (uint i = 0; i <= index; i++)
    ...
    // apaga as linhas com o pressionamento da tecla DELETE
    if (input->KeyPress(VK_DELETE))
    {
        // zera o índice e o contador
        index = count = 0;
        // vértices voltarão para a cor padrão
        vertexDefaultColor = true;
        // chama o display para apagar as linhas imediatamente
        Display();
    }
}
