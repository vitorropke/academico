...
def generate_interactive_map(instance: DataFrame, central_point: str, hubs: list[str], cycles: list[list[list[str]]],
                             filepath: str) -> None:
    m: Map = folium.Map(location=((instance['latitude'].max() + instance['latitude'].min()) / 2.0,
                                  (instance['longitude'].max() + instance['longitude'].min()) / 2.0), zoom_start=13,
                        control_scale=True)

    for index, latitude, longitude in zip(instance.index, instance['latitude'], instance['longitude']):
        folium.Marker(location=[latitude, longitude], tooltip=index, icon=folium.Icon()).add_to(parent=m)

    m.save(outfile=filepath)


def generate_static_map(instance: DataFrame, central_point: str, hubs: list[str], cycles: list[list[list[str]]],
                        filepath: str) -> None:
    imagery: OSM = OSM()
